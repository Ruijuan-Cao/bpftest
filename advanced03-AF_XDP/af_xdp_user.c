...
void map_get_value_array(int fd, __u32 key, struct datarec *value)
{
	if ((bpf_map_lookup_elem(fd, &key, value)) != 0) {
		printf("ERR: bpf_map_lookup_elem failed key:0x%X\n", key);
	}
}
...
__u32 key = XDP_PASS;
      ...
      //record.total.rx_packets = value.rx_packets;
      //record.total.saddr = value.saddr;
...
struct bpf_map_info info = { 0 };
	...
	if (verbose) {
		...
		/*	__u32 key = XDP_PASS;	
			struct datarec value;
			value.rx_packets = 121;
			int ret = bpf_map_update_elem(stats_map_fd, &key, &value, 0);
			if(ret)
				printf("update map fail\n");
		*/
		}
	}

	//pthread_t t;
	//int ret2 = pthread_create(&t, NULL, xxxx, (void *)stats_map_fd);
...
/*
	for(int i = 0;i < 10;i++)
	{
		__u32 key  = XDP_PASS;
		struct datarec value;
		bpf_map_lookup_elem(stats_map_fd, &key, &value);
		printf("%d---%lld---%x\n",i, value.rx_packets, value.saddr);
		sleep(1);
	}	
*/
	/* Cleanup */
